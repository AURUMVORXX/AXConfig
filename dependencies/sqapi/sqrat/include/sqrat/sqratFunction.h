...
public:
    ...
    ///
    /// \tparam Args... Variadic template arguments of the Function (usually doesnt need to be defined explicitly)
    ...
    SharedPtr<R> Evaluate(Args... args) {
        ...
        if (obj._type != OT_NATIVECLOSURE &&
            SQ_SUCCEEDED(sq_getclosureinfo(vm, -2, &nparams, &nfreevars)) && (nparams != sizeof...(Args) + 1)) {
            sq_pop(vm, 2);
...
#if !defined (SCRAT_NO_ERROR_CHECKING)
        SQRESULT result = sq_call(vm, sizeof...(Args) + 1, true, ErrorHandling::IsEnabled());
...
#if !defined (SCRAT_NO_ERROR_CHECKING)
        SQRESULT result = sq_call(vm, sizeof...(Args) + 1, false, ErrorHandling::IsEnabled());
        sq_settop(vm, top);
